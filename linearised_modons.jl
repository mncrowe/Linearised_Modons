...
include("linearised_QG.jl")

function run_modon(dev = GPU();
                linear = false,         # use linearised QG solver
             nonlinear = false,         # switch on nonlinear terms in linear solver
    ...
    ψ, q, ψx, ψy, qx, qy = build_modon(U, a, β, R, α, r, θ)

    # Build problem:

    if linear

        prob = LinQGProblem(dev;
                            nonlinear,
                            nx = Nx,
                            ny = Ny,
                            Lx,
    ...
    if linear
        q₀ = 1e-5 * Nx * irfft(to_dev(exp.(im*2π*randn(Int(Nx/2+1), Int(Ny)))).*(κ.>κ₁).*(κ.<κ₂), Nx)
        set_q!(prob, q₀)
    else
        q₀ = to_dev(q) .+ 1e-5 * Nx * irfft(to_dev(exp.(im*2π*randn(Int(Nx/2+1), Int(Ny)))).*(κ.>κ₁).*(κ.<κ₂), Nx)
    ...
    if save_window
        ...
        save_window_data(linear, prob, windowname, 0, ix, iy)
    ...
    for i1 in 1:ceil(Nt/I)
        ...
        x_c, y_c = get_centre(linear, prob, prob.grid)
        ...
        if (i1*I % Int(Nt/Nw) == 0) & save_window
            save_window_data(linear, prob, windowname, Int(i1*I*Nw/Nt), ix, iy)
        end
...
function save_window_data(linear, problem, filename, i, ix, iy)
    ...
    x_c, y_c = get_centre(linear, problem, problem.grid)
...
function get_centre(linear, problem, grid)
...
function build_modon(U, a, β, R, α, r, θ)
    ...
    ψx = @. ((Ψᵢ′(r) - Ψᵢ(r) / r) * (r < a) + (Ψₒ′(r) - Ψₒ(r) / r) * (r >= a)) * sin(θ - α) * cos(θ - α)
    qx = @. ((Qᵢ′(r) - Qᵢ(r) / r) * (r < a) + (Qₒ′(r) - Qₒ(r) / r) * (r >= a)) * sin(θ - α) * cos(θ - α)
    ψy = @. (Ψᵢ′(r) * (r < a) + Ψₒ′(r) * (r >= a)) * sin(θ)^2 + (Ψᵢ(r) * (r < a) + Ψₒ(r) * (r >= a)) / r * cos(θ - α)^2
    qy = @. (Qᵢ′(r) * (r < a) + Qₒ′(r) * (r >= a)) * sin(θ)^2 + (Qᵢ(r) * (r < a) + Qₒ(r) * (r >= a)) / r * cos(θ - α)^2
